...
# ==============================================================================
# 1. CONFIGURAÇÃO VISUAL (PREMIUM MEDICAL DESIGN - V4)
# ==============================================================================
st.set_page_config(
...
CLASS_TRANSLATION = {
    ...
    0: 'Peso Insuficiente', 1: 'Peso Normal', 2: 'Sobrepeso Nível I', 3: 'Sobrepeso Nível II',
    4: 'Obesidade Tipo I', 5: 'Obesidade Tipo II', 6: 'Obesidade Tipo III'
}

COLOR_MAP_PT = {
    'Peso Insuficiente': '#94a3b8', 'Peso Normal': '#10b981',
    'Sobrepeso Nível I': '#facc15', 'Sobrepeso Nível II': '#f97316',
...
def load_dataset_reference():
    try:
        ...
        df['Diagnóstico_PT'] = df[col_target].map(CLASS_TRANSLATION)
        df['Diagnóstico_PT'] = df['Diagnóstico_PT'].fillna(df[col_target])
        return df
    except:
        return None
...
def gerar_html_relatorio(paciente, idade, peso, altura, imc, diag, proba, habitos):
    ...
    <head>
        <style>
            ...
            .habits-list li::before {{ content: '•'; color: #3b82f6; position: absolute; left: 0; font-weight: bold; }}
    ...
    <body>
        <div class="report-card">
            <div class="header">
                <div><h1>FIAP-Med Analytics</h1><p style="margin:0; color:#64748b;">Relatório Clínico Inteligente</p></div>
            ...
            <div class="section-title">Análise de Diagnóstico</div>
            <div class="kpi-grid">
                <div class="kpi-box"><b>Diagnóstico Final</b><span>{diag}</span></div>
                <div class="kpi-box"><b>IMC</b><span>{imc:.1f} kg/m²</span></div>
                <div class="kpi-box"><b>Confiança IA</b><span>{proba:.1f}%</span></div>
            </div>
            <div class="section-title">Perfil Comportamental</div>
            <ul class="habits-list">
                <li><b>Dieta Calórica:</b> {habitos['favc']}</li>
                <li><b>Vegetais:</b> {habitos['fcvc']}</li>
                <li><b>Hidratação:</b> {habitos['ch2o']}</li>
...
if pipeline_obj is None:
    st.error("⚠️ ARQUIVOS .PKL NÃO ENCONTRADOS.")
    st.stop()
...
with st.sidebar:
    st.image("https://img.icons8.com/fluency/96/stethoscope.png", width=60) # Ícone V4
    ...
    with st.expander("👤 Dados Biométricos", expanded=True):
        ...
        family_hist = st.radio("Histórico Familiar de Obesidade", ["Sim", "Não"], horizontal=True, index=1)

    with st.expander("🥗 Alimentação", expanded=False):
        favc = st.radio("Dieta Hipercalórica/Gordurosa?", ["Sim", "Não"], horizontal=True, index=1)
        fcvc_label = st.select_slider("Frequência de Vegetais", ["Nunca", "Às vezes", "Sempre"], value="Sempre")
        ncp = st.slider("Refeições principais por dia", 1, 4, 3)
        caec_label = st.selectbox("Comer entre refeições", ["Não", "Às vezes", "Frequentemente", "Sempre"], index=0)
    ...
    with st.expander("🏃 Estilo de Vida", expanded=True):
        ...
        ch2o_real = st.number_input("Água (Litros por dia)", min_value=0.5, max_value=10.0, value=2.5, step=0.1)
        
        faf_options = {"Sedentário (0 dias)": 0.0, "Leve (1 a 2 dias/semana)": 1.0, "Moderado (3 a 4 dias/semana)": 2.0, "Intenso (5 ou mais dias/semana)": 3.0}
        faf_label = st.selectbox("Frequência de Exercícios", list(faf_options.keys()), index=2)
        
        tue_options = {"Baixo (0 a 2 horas/dia)": 0.0, "Médio (3 a 5 horas/dia)": 1.0, "Alto (Mais de 5 horas/dia)": 2.0}
        tue_label = st.selectbox("Uso de Telas (Celular/PC)", list(tue_options.keys()), index=0)
        
        calc_label = st.selectbox("Consumo de Álcool", ["Não", "Às vezes", "Frequentemente", "Sempre"], index=0)
        mtrans_label = st.selectbox("Meio de Transporte", ["Caminhada", "Transporte Público", "Automóvel", "Moto", "Bicicleta"], index=0)

# Mapeamentos e DataFrame Input
map_gen = {"Masculino": "Male", "Feminino": "Female"}
map_yn = {"Sim": "yes", "Não": "no"}
...
map_mtrans = {"Transporte Público": "Public_Transportation", "Automóvel": "Automobile", "Caminhada": "Walking", "Moto": "Motorbike", "Bicicleta": "Bike"}
...
st.markdown("""
    <div class="fiap-header">
        <h1>🩺 FIAP-Med Analytics</h1>
        <p>Sistema Inteligente de Apoio à Decisão Clínica</p>
    </div>
""", unsafe_allow_html=True)

# LÓGICA DO BOTÃO "PROCESSAR"
if st.button("🚀 PROCESSAR ANÁLISE COMPLETA", type="primary", use_container_width=True):
...
if st.session_state.processed:
    ...
    st.markdown(f"""
        <div class="kpi-container">
            <div class="kpi-card">
                <div class="kpi-label">Diagnóstico Final</div>
            ...
            <div class="kpi-card">
                <div class="kpi-label">Confiança IA</div>
                <div class="kpi-value">{data['confianca']:.1f}%</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    
    tab1, tab2, tab3 = st.tabs(["📍 Diagnóstico & IA", "🌌 Universo & Segmentação", "📄 Relatório & Impressão"])
    ...
    with tab1:
        ...
        if is_discrepant:
            st.warning(f"💡 **Insight Clínico:** Detectamos uma divergência. Pelos hábitos, a IA sugere **{top1_nome}**, mas a biometria confirma **{diag_final_str}**.")
        
        st.markdown("---")
        
        g1, g2 = st.columns([2, 1])
        with g1:
            st.markdown("#### 🧬 Padrões de Probabilidade")
    ...
    # --- ABA 2 (VISUAL V4, LÓGICA V3) ---
    with tab2:
        st.markdown("### 🔭 Onde você está em relação aos outros pacientes?")
        if df_ref is not None:
            with st.expander("🔍 FILTRAR UNIVERSO DE COMPARAÇÃO", expanded=True):
                col_f1, col_f2, col_f3, col_f4 = st.columns(4)
                f_gender = col_f1.checkbox(f"Apenas {inputs['gender']}s", value=False, key="fg1")
                f_family = col_f2.checkbox("Mesmo Histórico Familiar", value=False, key="fg2")
                f_smoke = col_f3.checkbox("Mesmo Status de Fumante", value=False, key="fg3")
                f_active = col_f4.checkbox("Atividade Física Similar", value=False, key="fg4")
            ...
            if n_amostra > 0:
                st.markdown(f"**Comparando com:** `{n_amostra}` pacientes compatíveis.")
                try:
                    # Gráfico Scatter V4
                    fig_uni = px.scatter(
                        df_filtered, x='Weight', y='Height', color='Diagnóstico_PT', 
                        labels={'Weight': 'Peso (kg)', 'Height': 'Altura (m)', 'Diagnóstico_PT': 'Diagnóstico'},
                    ...
                    fig_uni.add_trace(go.Scatter(
                        ...
                        name='VOCÊ', text=['VOCÊ'], textposition='top center',
                ...
                except Exception as e: st.error(f"Erro no gráfico: {e}")
            else:
                st.warning("Nenhum paciente encontrado com essa combinação exata de filtros.")
        else: st.warning("Arquivo 'Obesity.csv' não encontrado.")

    # --- ABA 3 (RELATÓRIO V4) ---
    with tab3:
        st.markdown("### 📄 Relatório Médico")
        st.info("Relatório pronto para download.")
        habitos_display = {
            'favc': "Sim" if inputs['favc']=="Sim" else "Não",
            'fcvc': inputs['fcvc'],
            'ch2o': f"{inputs['ch2o']} Litros",
            'faf': inputs['faf_lbl'].split(" (")[0],
            'tue': inputs['tue'].split(" (")[0],
            'smoke': "Sim" if inputs['smoke'] else "Não"
        }
        ...
        # Botão V4
        href = f'<a href="data:file/html;base64,{b64}" download="Relatorio_FIAP_{inputs["nome"]}.html" style="text-decoration:none; width:100%;"><button style="background-color:{COLOR_SUCCESS}; color:white; padding:15px; width:100%; border-radius:8px; border:none; font-weight:bold; cursor:pointer;">📥 BAIXAR RELATÓRIO PDF/HTML</button></a>'
        c1, c2 = st.columns([1, 2])
        c1.markdown(href, unsafe_allow_html=True)
        components.html(html_report, height=600, scrolling=True)

else:
    st.info("👈 Preencha os dados e clique em PROCESSAR.")